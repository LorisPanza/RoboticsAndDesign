...
long value_pet; //value read by the capacitive sensor for detecting petting
int treshold_pet=40; //treshold over the which I recognize a petting action
...
bool transition_shoulder_happy_petting_shoulder(){
  value_pet = ADCTouch.read(A2) - ref_pet;
  Serial.print("Value read by the petting capacitive sensor: ");Serial.println(value_pet);
  if(value_pet > treshold_pet){
